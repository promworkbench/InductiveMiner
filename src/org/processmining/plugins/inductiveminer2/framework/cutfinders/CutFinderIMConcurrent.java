...
public class CutFinderIMConcurrent implements CutFinder {
	...
	public static Cut findCutImpl(IMLogInfo logInfo, Function<Integer, MultiIntSet> minimumSelfDistanceBetween) {
		...
		for (int e1 = 0; e1 < numberOfActivities; e1++) {
			for (int e2 = e1 + 1; e2 < numberOfActivities; e2++) {
	...
	public static List<TIntSet> ensureStartEndInEach(NormalisedIntDfg dfg, Collection<TIntSet> connectedComponents) {
		...
		for (TIntSet cc : connectedComponents) {
			...
			if (Sets.intersection(cc, dfg.getStartActivityIndices()).size() == 0) {
				hasStart = false;
			}
			Boolean hasEnd = true;
			if (Sets.intersection(cc, dfg.getEndActivityIndices()).size() == 0) {
				hasEnd = false;
			}
			if (hasStart) {
				if (hasEnd) {
